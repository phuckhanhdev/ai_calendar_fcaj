...
export default function AdminPage() {
  ...
  const handleTogglePremium = async (targetUserId: string, currentStatus: string) => {
    ...
    // Process (Approve / Reject) Pending Transactions
                        ...
                        <tr key={u.id} className="admin-table-row">
                          ...
                          <td className="admin-table-cell text-right space-x-1.5">
                            ...
                            <button
                              ...
                              className={`admin-btn-action ${u.subscriptionStatus === "premium" ? "danger" : "primary"
                                }`}
                            >
                        ...
                        transactions.map((t) => (
                          <tr key={t.id} className="admin-table-row">
                            ...
                            <td className="admin-table-cell text-right">
                              <span className={`admin-badge ${t.status === "completed" ? "emerald" : t.status === "pending" ? "amber" : "rose"}`}>
                                {t.status === "completed" ? "Đã duyệt" : t.status === "pending" ? "Chờ duyệt" : "Từ chối"}
                              </span>
                              {t.status === "pending" && (
                                <div className="mt-1.5 flex justify-end gap-1">
                                  <button
                                    onClick={() => handleProcessTransaction(t.id, "approve")}
                                    className="admin-btn-action primary"
                                    title="Duyệt giao dịch và kích hoạt Premium"
                                  >
                                    Duyệt
                                  </button>
                                  <button
                                    onClick={() => handleProcessTransaction(t.id, "reject")}
